function NtmtTesterDial(styleId,typ) {
	...
	this.dWrite('<div id="'+this.htmlId+'" style="width:'+this.width+'px;display: inline-block;"height:'+this.height+'px">');
	...
	$('#'+this.htmlId ).fadeTo( 10 , 0.25, function() {
	});
	this.drawDial = function(value) {
		...
		jQuery({someValue: (this.valueOld/1024) ,  htmlID:this.htmlId }).animate({someValue: (value/1024)}, {
			...
			$('#'+this.htmlId+' .mtrwebtester_dial_text_number').text('&nbsp;');
		}
		this.valueOld=value;
	}
...
function NtmtTesterDialPing(styleId) {
	...
	this.dWrite('<div id="'+this.htmlId+'" style="width:'+this.width+'px;height:'+this.height+'px;position:relative;">');
